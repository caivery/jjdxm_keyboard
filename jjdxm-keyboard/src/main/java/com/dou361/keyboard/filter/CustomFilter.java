package com.dou361.keyboard.filter;
...
import com.dou361.keyboard.listener.EmojiDisplayListener;
import com.dou361.keyboard.parse.CustomEmoticons;
import com.dou361.keyboard.ui.EmoticonSpan;
import com.dou361.keyboard.utils.EmoticonsKeyboardUtils;

import java.util.regex.Matcher;
import java.util.regex.Pattern;

public class CustomFilter extends EmoticonFilter {

    public static final int WRAP_DRAWABLE = -1;
    private int emoticonSize = -1;
    public static final Pattern Custom_RANGE = Pattern.compile("\\[[a-zA-Z0-9\\u4e00-\\u9fa5]+\\]");

    public static Matcher getMatcher(CharSequence matchStr) {
        return Custom_RANGE.matcher(matchStr);
    }
    ...
    public void filter(EditText editText, CharSequence text, int start, int lengthBefore, int lengthAfter) {
        ...
        if (m != null) {
            while (m.find()) {
                String key = m.group();
                int icon = CustomEmoticons.sCustomEmoticonHashMap.get(key);
                if (icon > 0) {
    ...
    public static Spannable spannableFilter(Context context, Spannable spannable, CharSequence text, int fontSize, EmojiDisplayListener emojiDisplayListener) {
        if (CustomEmoticons.sCustomEmoticonHashMap.size() <= 0) {
        ...
        if (m != null) {
            while (m.find()) {
                String key = m.group();
                int icon = 0;
                if(CustomEmoticons.sCustomEmoticonHashMap.get(key)!= null){
                    icon = CustomEmoticons.sCustomEmoticonHashMap.get(key);
                }
